...
def main(X,Y, dx):
    ...
    for train_index, test_index in skf.split(X, Y[:, -1]):
        ...
        model = lstm_model(input_shape = (None, 19), stateful = stateful, target_rep = target_rep, bidirectional = bidirectional)
        ...
        for epoch in range(30):
            ...
            for seq, label in zip(X_train, y_train):
                #sleep (0.0001)
                loss, acc = model.train_on_batch(np.array([seq]), [label.reshape(1,25)])
                tr_loss.append(loss)
                sys.stdout.write('\r')
                sys.stdout.write("[%-20s] %d%%" % ('='* int((count+1)/(len(X_train)/20)), int(5*(count+1))/(len(X_train)/20)))
                sys.stdout.write(', Acc = %.3f' % (acc))
                sys.stdout.write(', Loss = %.3f' % (loss))
                sys.stdout.flush()
                count +=1
                
            for seq, label in zip(X_train, y_train):   
                yhat = model.predict_on_batch(np.array([seq]))
                y_pred.append(yhat.reshape(label.shape))
            y_pred = np.array(y_pred)
            
            for idx in range(Y[0].shape[0]):
                fpr[idx], tpr[idx], _ = roc_curve(y_train[:, idx], y_pred[:, idx])
                tr_roc_auc[idx] = auc(fpr[idx], tpr[idx])
                tr_matrix[idx] = confusion_matrix(y_train[:, idx], np.array([round(i) for i in y_pred[:, idx]]))
            ...
            for idx in range(Y[0].shape[0]):
                ...
                te_matrix[idx] = confusion_matrix(y_test[:, idx], np.array([round(i) for i in y_pred[:, idx]]))
...
def lstm_model(input_shape, stateful = False, target_rep = False, bidirectional = False):
    ...
    if (bidirectional == True):
        if (stateful == True):
            model.add(Bidirectional(LSTM(1024, return_sequences = True, stateful =stateful), merge_mode = 'concat', batch_input_shape = (1, input_shape[0], input_shape[1]),))
        else:
            model.add(Bidirectional(LSTM(1024, return_sequences = True, stateful = stateful), merge_mode = 'concat', input_shape = input_shape))
    else:
        if (stateful == True):
            model.add(LSTM(1024, batch_input_shape = (1, input_shape[0], input_shape[1]), return_sequences=True, stateful=stateful))
        else:
            model.add(LSTM(1024, input_shape = input_shape, return_sequences=True, stateful=stateful))
...
def hierarchal_lstm (input_shape, embedding, stateful= True, target_rep = False, bidirectional = True):
